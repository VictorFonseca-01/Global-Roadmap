...
import { User, Mail, Briefcase, Building, Shield, Save, Camera, Palette, LogOut, Loader2 } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const updateMutation = useMutation({
    mutationFn: (values: ProfileFormValues) => userService.updateProfile(profile!.id, values),
    onSuccess: () => {
  ...
  const avatarMutation = useMutation({
    mutationFn: (file: File) => userService.uploadAvatar(profile!.id, file),
    onSuccess: (url) => {
      userService.updateProfile(profile!.id, { avatar_url: url });
      queryClient.invalidateQueries({ queryKey: ["user-profile"] });
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        ...
        <Button 
          ...
          className="rounded-full px-8 h-12 shadow-lg shadow-primary/20 font-bold"
        >
          ...
          <Card className="rounded-[2.5rem] border-none shadow-xl overflow-hidden bg-white dark:bg-slate-900">
            <CardContent className="p-8 flex flex-col items-center text-center">
              <div className="relative group mb-6">
                <div className="h-32 w-32 rounded-full overflow-hidden border-4 border-primary/20 group-hover:border-primary transition-all duration-300 bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
                  {uploading ? (
                    <Loader2 className="h-8 w-8 animate-spin text-primary" />
                  ) : profile?.avatar_url ? (
                ...
                <button 
                  ...
                  disabled={uploading}
                  className="absolute bottom-0 right-0 p-2 bg-primary text-white rounded-full shadow-lg hover:scale-110 transition-all disabled:opacity-50"
              ...
              <div className="w-full space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">
                    {profile?.last_login ? format(new Date(profile.last_login), "dd/MM/yyyy HH:mm", { locale: ptBR }) : '---'}
                  </span>
                </div>
              </div>
            </CardContent>
              ...
              <CardContent className="p-8 pt-0 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label htmlFor="full_name" className="text-sm font-bold ml-1">Nome Completo</Label>
                    <div className="relative">
                      <User className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                      <Input 
                        id="full_name" 
                        {...register("full_name")}
                        className={`rounded-2xl h-12 pl-12 bg-slate-50 dark:bg-slate-950 border-none focus-visible:ring-primary ${errors.full_name ? 'ring-2 ring-red-500' : ''}`} 
                      />
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input 
                        id="email" 
                        {...register("email")}
                        className={`rounded-2xl h-12 pl-12 bg-slate-50 dark:bg-slate-950 border-none focus-visible:ring-primary ${errors.email ? 'ring-2 ring-red-500' : ''}`} 
                      />
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input 
                        id="role" 
                        {...register("role")}
                        className={`rounded-2xl h-12 pl-12 bg-slate-50 dark:bg-slate-950 border-none focus-visible:ring-primary ${errors.role ? 'ring-2 ring-red-500' : ''}`} 
                      />
                    </div>
                    {errors.role && <p className="text-xs text-red-500 ml-1">{errors.role.message}</p>}
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="department" className="text-sm font-bold ml-1">Departamento</Label>
                    <div className="relative">
                      <Building className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                      <Input 
                        id="department" 
                        {...register("department")}
                        className={`rounded-2xl h-12 pl-12 bg-slate-50 dark:bg-slate-950 border-none focus-visible:ring-primary ${errors.department ? 'ring-2 ring-red-500' : ''}`} 
                      />
                    </div>
            ...
            <CardContent className="p-8 pt-0 space-y-6">
              <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-950 rounded-2xl">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Alternar entre modo claro e escuro manualmente ou pelo sistema.</p>
                </div>
                <div className="flex gap-1 bg-white dark:bg-slate-900 p-1 rounded-xl shadow-inner border border-slate-100 dark:border-slate-800">
                  <Button 
                    ...
                    size="sm" 
                    className={`rounded-lg h-8 px-4 font-bold ${theme === "light" ? "shadow-sm" : ""}`}
                  ...
                  <Button 
                    ...
                    size="sm" 
                    className={`rounded-lg h-8 px-4 font-bold ${theme === "dark" ? "shadow-sm" : ""}`}
                  ...
                  <Button 
                    ...
                    size="sm" 
                    className={`rounded-lg h-8 px-4 font-bold ${theme === "system" ? "shadow-sm" : ""}`}
