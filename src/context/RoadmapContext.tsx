...
const loadSwimlanes = (year: number) => {
  const saved = localStorage.getItem(`roadmap_swimlanes_${year}`);
  if (saved) {
    try {
...
export function RoadmapProvider({ children }: { children: ReactNode }) {
  ...
  const [swimlanes, setSwimlanes] = useState<Swimlane[]>(() => loadSwimlanes(initialYear));
  const [items, setItems] = useState<RoadmapItem[]>(() => loadItems(initialYear));
  ...
  const setYear = (newYear: number) => {
    ...
    setSwimlanes(loadSwimlanes(newYear));
    setItems(loadItems(newYear));
  };
