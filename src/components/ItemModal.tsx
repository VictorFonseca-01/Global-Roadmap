...
export function ItemModal({ item, onClose }: Props) {
  ...
  const [status, setStatus] = useState<NonNullable<RoadmapItem['status']>>(item.status || 'on_track');
  const [priority, setPriority] = useState<NonNullable<RoadmapItem['priority']>>(item.priority || 'medium');
          ...
          <div className="p-6 space-y-4 overflow-y-auto flex-1">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select 
                  value={status}
                  onChange={e => setStatus(e.target.value as NonNullable<RoadmapItem['status']>)}
              ...
              <div>
                ...
                <select 
                  value={priority}
                  onChange={e => setPriority(e.target.value as NonNullable<RoadmapItem['priority']>)}
                  className="w-full border border-slate-300 dark:border-slate-800 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent dark:bg-slate-950 dark:text-slate-100"
                >
